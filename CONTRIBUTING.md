...
Black uses [Tendermint’s coding repo](https://github.com/tendermint/coding) for overall information on repository
...
1. Either [open](https://github.com/xblackfury/black/issues/new/choose)
   or [find](https://github.com/xblackfury/black/issues) an issue you have identified and would like to contribute to
   resolving.
...
3. If you would like to contribute:
    ...
    8. Please make sure to run `make format` before every commit - the easiest way to do this is having your editor run
       ...
       . There are CI tests built into the Black repository and all PR’s will require that these tests pass before
...
some [good first issues](https://github.com/xblackfury/black/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Black, please create
an [ADR](https://github.com/xblackfury/black/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/xblackfury/black`, the code should never exist
at `$GOPATH/src/github.com/xblackfury/black`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/xblackfury/black`, and do all the work there.

For instance, to create a fork and work on a branch of it, you would:

1. Create the fork on github, using the fork button.
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/xblackfury/black`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:ivanshukhov/black.git`
...
4. Create your feature branch from `development` either on `github.com/xblackfury/black`, or your fork (
...
**Note**: Black's team currently cuts releases on a need to have basis. We will announce a more
