...
speed and transaction amount. For Black, we are talking about close to
...
I believe this was disabled by default on Black, but listing it in any
...
will risk database corruption if the `blackd` is killed for any reason.
...
Below is the disk usage after two weeks of Black Arsia Mons testnet. The default
