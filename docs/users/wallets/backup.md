...
blackd keys add dev0
{
  "name": "dev0",
  "type": "local",
  "address": "did:fury:1n253dl2tgyhxjm592p580c38r4dn8023ctv28d",
...
```bash
$ blackd keys add dev0-restored --recover
...
```bash
blackd keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ blackd keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ blackd keys unsafe-import-eth-key dev0-imported ./dev0.export
