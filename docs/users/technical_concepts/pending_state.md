...
Learn how Black handles pending state queries. {synopsis}
...
Black is designed quite differently on this front as there is no concept of a "pending state".
Black uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Black cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool/).

## Pending State Queries

Black will make queries which will account for any unconfirmed transactions present in a node's
