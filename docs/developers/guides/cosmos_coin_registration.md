...
Learn how to register native Cosmos Coins through [Black Governance](../../users/governance/overview.md). {synopsis}
...
## Adding Network to Black Chain Registry

All chain registry information can be found [here](https://github.com/evmos/chain-token-registry). The [Cosmos Chain Registry](https://github.com/cosmos/chain-registry) is used to pull in the list of RPC, gRPC, and REST endpoints to power the Black Dashboard. To add chain registry information, please submit a pull request with the network details. Pull requests should be merged one business day after governance passes.
...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`blackd`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
blackd tx gov submit-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
blackd tx gov submit-proposal register-coin --proposal=<path/to/proposal.json>
```
